...
function buildBeginnerTermDetail(term) {
  const difficultyText = {
    easy: '처음 배울 때 꼭 잡아두면 좋은 기본 개념입니다.',
    medium: '기본 개념을 알고 난 뒤 연결해서 이해하면 좋은 개념입니다.',
    hard: '처음에는 어렵게 느껴질 수 있으니 예시와 함께 천천히 익히면 좋습니다.',
  }[term.difficulty] || '코딩을 이해할 때 자주 만나는 개념입니다.';
  const hint = term.hint || '핵심 힌트를 떠올리며 의미를 연결해보세요.';

  return [
    `${term.term}이라는 말은 ${term.category}에서 자주 쓰이는 용어입니다. 먼저 짧게 외우기보다 "${hint}"처럼 머릿속에 이미지를 만들어두면 기억하기 쉽습니다.`,
    `기본 설명은 "${term.definition}"입니다. 초보자 입장에서는 이 문장을 "언제 쓰는가"와 "무엇을 도와주는가"로 나누어 보면 좋습니다.`,
    `${difficultyText} 문제를 풀 때 이 설명이 나오면, 보기에서 가장 가까운 역할을 하는 단어가 무엇인지 찾는 방식으로 접근하세요.`,
  ];
}
